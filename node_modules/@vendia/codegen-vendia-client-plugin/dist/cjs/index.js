...
var GraphQLRequestVisitor = class extends import_visitor_plugin_common.ClientSideBaseVisitor {
  ...
  OperationDefinition(node) {
    ...
    if (!operationName) {
      console.warn(`Anonymous GraphQL operation was ignored in "typescript-graphql-request", please make sure to name your operation: `, (0, import_graphql.print)(node));
      return null;
    }
...
var plugin = (schema, documents, config) => {
  ...
  const allFragments = [
    ...allAst.definitions.filter((d) => d.kind === import_graphql2.Kind.FRAGMENT_DEFINITION).map((fragmentDef) => ({
      node: fragmentDef,
