...
var compileTs = async (entry, options) => {
...
function generateListOperation({ entity, entities, schemaNamespace }) {
  ...
  return `
          query ${entity}${queryNameSeparator}list($nextToken: String, $limit: Int, $filter: ${filterType}) {
            list${entityPlural}(nextToken: $nextToken, limit: $limit, filter: $filter) {
...
var generateClient = async ({
  ...
  await compileTs(indexFilePath, { outDir: dotVendiaClientPath, tsconfig: tsconfigPath, dts: true });
  const tsconfigModulePath = import_path.default.join(installedClientPath, "tsconfig.module.json");
  await compileTs(indexFilePath, {
